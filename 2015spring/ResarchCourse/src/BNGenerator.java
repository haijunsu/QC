//================================================================
//     Copyright (c) 2003, Escola PolitEcnica-USP
//                     All Rights Reserved
//================================================================
//
// NAME : BNGenerator
 ...
 * 3) Instead of saving directly in a file, it creates a JavaBayes graphical object and save it in a corresponding format
 * 4)
 *
 *
 */
...
public class BNGenerator {
    ...
    public static void main(String argv[]) throws Exception {
        ...
        if( argv.length > 0 ) {
            for( int i = 0; i < argv.length; i++ ) {
                ...
                // format of the saved network
                    ...
                    i++;
                    format = argv[i];
                    if ( (format.compareTo("xml") !=0) && (format.compareTo("java") !=0) &&
                ...
                // help option
                    ...
                    generate=false;
                    System.out.println();
                    System.out.println("BNGenerator - a generator for random Bayesian network. Version (0.3)");
                    auxBN.printHelp();
                    System.exit(0);
                }
                else    {
        ...
        System.out.println("Bayesian networks generated!");
        System.exit(0);
    ...
    public void printHelp(){
        ...
        System.out.println("\tMaximum induced-width allowed: [-maxIW value]");
        System.out.println("\tMaximum number of value for each node: [-maxVal value]");
        System.out.println("\tThis option fix the number of values to the maximum value: [-fixed_maxVal]");
    ...
    String testU, String format, String baseFileName)	throws Exception	{
        ...
        for (int g = 0; g < nGraphs; g++) {
            ...
            for (int i=0;i<nIterations;i++ )  {
                ...
                //} // for coherency
                //else {
                // System.out.println("\t********  MATRIXS NOT COHERENT!! ********** ");
                //  System.exit(0);
                //} // end of else
                //pause();
 ...
 */
    ...
    void ckeckingCycle(int nGra,int nIterati,int maxVal,String te,String fo,String baseFileNam,int maxInducedWidt) throws Exception{
        ...
        for (semente=1;semente<10000;semente++){
            ...
            if  (iwDistribution[3]<900)  {
                System.out.println("A semente geradora de ciclo E: "+semente);
                outputFile.println("A semente procurada geradora de ciclo E: "+semente);
    ...
    private void saveBNBIF(String name,String format,int n, int maxValues) throws Exception { // Save BN in XML format
        String fileName = name.concat("" + (n+1) + "." + format);
        PrintStream outputFile = new PrintStream(new FileOutputStream(fileName), true);
        
        outputFile.println("// \tBayesian network in BIF(BayesNet Interchange Format)");
        ...
        for (int i=0;i<nNodes;i++ ) {
            int nParents=1;
            outputFile.println(" probability \"node"+i+"</FOR>");
            if (parentMatrix[i][0]!=1)  {
                int aux=parentMatrix[i][0];
                for (int j=1;j<aux;j++) {
                    outputFile.println("      <GIVEN>node"+parentMatrix[i][j]+"</GIVEN>");
 ...
 */
    ...
    private void printArcs()  {  // just print the arcs
