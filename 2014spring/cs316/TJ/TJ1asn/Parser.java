...
public final class Parser {
  ...
  private static void methodDecl() throws SourceFileErrorException
  {
    ...
    /* ???????? */
    /* Haijun Su */
    /* static ( void | int { '[' ']' }) IDENTIFIER '(' <parameterDeclList> ')' <compoundStamt> */
  ...
  private static void assignmentOrInvoc() throws SourceFileErrorException
  {
    ...
    /* ???????? */
    /* Haijun Su */
    /* IDENTIFIER ( { '[' <expr3> ']' } = <expr3> ; | <argumentList> ; ) */
    accept(IDENT); //IDENTIFIER
    if (getCurrentToken() == LBRACKET) { //[
    	...
    	expr3(); //expr3
    	accept(SEMICOLON); //;
    } else if (getCurrentToken() == BECOMES) {//=
    	nextToken();
    	expr3(); //expr3
    	accept(SEMICOLON); //;
    } else {
  ...
  private static void outputStmt() throws SourceFileErrorException
  {
    ...
    */
    /* Haijun Su */
    /* System . out . ( print '(' <printArgument> ')' ; | println '(' [ <printArgument> ] ')' ; */
    switch (getCurrentToken()) {
    	case PRINT:
    		nextToken(); //print
    		accept(LPAREN); //(
