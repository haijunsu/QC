...
public final class ParserAndTranslator {
	...
	private static void accept(Symbols expectedToken)
			throws SourceFileErrorException {
		if (getCurrentToken() == expectedToken)
			nextToken();
		else
			throw new SourceFileErrorException(
					"Something's wrong--maybe the following token is missing: "
							+ expectedToken.symbolRepresentationForOutputFile);
	}

	private static MethodRec lookUpCalledMethod(String methodName,
			int expectedType) throws SourceFileErrorException {
		...
		if (t == null) { /* the method has never been declared nor called */
			...
			throw new SourceFileErrorException(
					"Method does not have appropriate return type");
	...
	static void program() throws SourceFileErrorException {
		...
		if (MethodRec.getUndeclaredMethodCount() != 0)
			throw new SourceFileErrorException(
					"Not all called methods have been declared");
	...
	private static void varDecl() throws SourceFileErrorException {
		...
		if (getCurrentToken() == INT) {
			...
			if (!ScannerImported)
				throw new SourceFileErrorException(
						"Scanner used without having been imported");

			nextToken();

			if (getCurrentToken() == IDENT) {
				if (level == 1) {
					if (BlockRec.searchForStatic(
							LexicalAnalyzer.getCurrentSpelling(), false) != null)
						throw new SourceFileErrorException(
								"Illegal redeclaration of class variable "
										+ LexicalAnalyzer.getCurrentSpelling());
					else
						new ClassVariableRec(
								LexicalAnalyzer.getCurrentSpelling());
				} else {
					if (symTab.searchForLocal(LexicalAnalyzer
							.getCurrentSpelling()) != null)
						throw new SourceFileErrorException(
								"Illegal redeclaration of local variable "
										+ LexicalAnalyzer.getCurrentSpelling());
					else
						new LocalVariableRec(
								LexicalAnalyzer.getCurrentSpelling());
				}
				nextToken();
			} else
		...
		if (getCurrentToken() == IDENT) {
			...
			if (level == 1) {
				if (BlockRec.searchForStatic(identName, false) != null)
					...
					v = new ClassVariableRec(identName, VariableRec.INT,
							dimensions, addr);
				...
				if (symTab.searchForLocal(identName) != null)
					...
					v = new LocalVariableRec(identName, VariableRec.INT,
							dimensions, stackFrameOffset);
	...
	private static void mainDecl() throws SourceFileErrorException {
		...
		INITSTKFRMinstr iInstr = new INITSTKFRMinstr(
				Instruction.OPERAND_NOT_YET_KNOWN);

		compoundStmt(t, null);

		iInstr.fixUpOperand(t.getTable().getMaxNextOffset()
				- TJ.FIRST_LOCALVAR_STACKFRAME_OFFSET);
	...
	private static void methodDecl() throws SourceFileErrorException {
		...
		if (getCurrentToken() == IDENT) {
			t = (MethodRec) BlockRec.searchForStatic(
					LexicalAnalyzer.getCurrentSpelling(), true);

			if (t == null) /*
							 * no call or declaration of this method has yet
							 * been seen
							 */
				t = new MethodRec(LexicalAnalyzer.getCurrentSpelling(), type,
						dimensions);

			else if (t.getCallsToBeFixedUp() != null) {/*
				...
				throw new SourceFileErrorException("Method "
						+ LexicalAnalyzer.getCurrentSpelling()
		...
		if (t.getArgCount() == MethodRec.NOT_KNOWN) // Method has not yet been
													// called
			t.setArgCount(parameters.paramCount);
		else if (t.getArgCount() != parameters.paramCount)
			throw new SourceFileErrorException("Method " + t.name
					+ " was previously called with " + t.getArgCount()
					+ " arguments");
	...
	private static ParameterList parameterDeclList()
			throws SourceFileErrorException {
	...
	private static void parameterDecl(ParameterList p)
			throws SourceFileErrorException {
	...
	private static void compoundStmt(MethodRec m, IdentList params)
			throws SourceFileErrorException {
		...
		if (m == null)
			...
			for (IdentList p = params; p != null; p = p.next) {
				if (symTab.searchForLocal(p.name) != null)
					throw new SourceFileErrorException(
							"Illegal redeclaration of parameter " + p.name);
				else
					new LocalVariableRec(p.name, VariableRec.INT, p.dimensions,
							--offset);
			}
		}
	...
	private static void statement() throws SourceFileErrorException {
		...
		case RETURN:
			...
			if (((CompoundStmtBlockRec) symTab).methodBlock.methodRec.getType() == MethodRec.INT)
				expr3();
			new RETURNinstr(
					((CompoundStmtBlockRec) symTab).methodBlock.methodRec
							.getArgCount());
			accept(SEMICOLON);
			break;
		case INT:
		...
		default:
			throw new SourceFileErrorException(
					"Expected first token of a <statement>, not "
	...
	private static void assignmentOrInvoc() throws SourceFileErrorException {
		...
		if (getCurrentToken() != LPAREN) {
			...
			if (t == null)
				throw new SourceFileErrorException("Undeclared variable: "
						+ identName);
			...
			CALLSTATMETHODinstr cInstr = new CALLSTATMETHODinstr(
					t.getStartAddr());

			if (t.getStartAddr() == Instruction.OPERAND_NOT_YET_KNOWN)
				/* the method has not yet been declared */
				if (t.getType() == MethodRec.NOT_KNOWN)
					t.setCallsToBeFixedUp(new FixUpRec(cInstr, t
							.getCallsToBeFixedUp(),
					...
					t.setCallsToBeFixedUp(new FixUpRec(cInstr, t
							.getCallsToBeFixedUp()));
					if (t.getType() != MethodRec.VOID)
						new NOPorDISCARDVALUEinstr(false); // generate
															// DISCARDVALUE
															// instruction
				}

			else if (t.getType() != MethodRec.VOID)
				new NOPorDISCARDVALUEinstr(false); // generate DISCARDVALUE
													// instruction
	...
	private static void argumentList(MethodRec m)
			throws SourceFileErrorException {
	...
	private static void ifStmt() throws SourceFileErrorException {
		...
		JUMPONFALSEinstr jInstr1 = new JUMPONFALSEinstr(
				Instruction.OPERAND_NOT_YET_KNOWN);
	...
	private static void whileStmt() throws SourceFileErrorException {
		...
		/* ???????? */
		/* Haijun Su */
	    /* while '(' <expr7> ')' <statement> */
	    accept(WHILE); //while
	    accept(LPAREN); // (
		int loopa = Instruction.getNextCodeAddress();
	    expr7(); //<expr7>
	    accept(RPAREN); //)
		JUMPONFALSEinstr jInstr1 = new JUMPONFALSEinstr(
				Instruction.OPERAND_NOT_YET_KNOWN);
	...
	private static void outputStmt() throws SourceFileErrorException {
		...
		/* Haijun Su */
		/*
		 * System . out . ( print '(' <printArgument> ')' ; | println '(' [
		 * <printArgument> ] ')' ;
		 */
		case PRINT:
			nextToken(); // print
	...
	private static void printArgument() throws SourceFileErrorException {
		...
		if (getCurrentToken() == CHARSTRING) {
			new WRITESTRINGinstr(LexicalAnalyzer.getStartOfString(),
					LexicalAnalyzer.getEndOfString());
			nextToken(); // CHARSTRING
		} else {
			expr3(); // <expr3>
			new WRITEINTinstr();
		}
	...
	private static void expr2() throws SourceFileErrorException {
		...
		while (getCurrentToken() == TIMES || getCurrentToken() == DIV
				|| getCurrentToken() == MOD) {
