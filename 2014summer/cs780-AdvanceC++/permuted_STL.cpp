// Inverted Index with the STL (map)
...
// CS381/780 Project 10 Submission: Su, Haijun
...
int main(int argc, char* argv[]){
    ...
    string line;
	long lineNum = 0;
	map<string, WordIndex> wiMap;
	while (getline(input, line)){
		++lineNum;
		stringstream ss(line);
		string buf;
		while (ss >> buf) {
			if (wiMap[buf].getWord() != buf) {
				WordIndex wi2(buf);
				wiMap[buf] = wi2;
			}
			wiMap[buf].addLineNumber(lineNum);
		}
	}
	for (map<string, WordIndex>::iterator it = wiMap.begin(); it != wiMap.end(); ++it) {
		output<<(*it).second;
		cout << (*it).second;
	}
	input.close();
